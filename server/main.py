...
def analysis_result():
    ...
    if sortby == 'default':
        sql_results = db.execute('select * from strategy order by strategy_id desc').fetchall()
    elif sortby == 'myself':
        sql_results = db.execute('select * from strategy where author=? order by strategy_id desc', [session['USERNAME']]).fetchall()
    elif sortby == 'return':
        sql_results = db.execute('select * from strategy order by return desc').fetchall()
    elif sortby == 'vol':
        sql_results = db.execute('select * from strategy order by volatility asc').fetchall()
    elif sortby == 'sharpe':
        sql_results = db.execute('select * from strategy order by sharpe_ratio desc').fetchall()
    elif sortby == 'mdd':
        sql_results = db.execute('select * from strategy order by max_drawdown asc').fetchall()
    db.close()    
    return render_template('result.html', results=sql_results)
